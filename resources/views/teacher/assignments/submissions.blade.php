...
<div class="card mb-3">
    <div class="card-body">
        <p class="mb-1"><strong>Subject:</strong> {{ $assignment->subject->name ?? '-' }} &nbsp;|&nbsp; <strong>Total Marks:</strong> {{ $assignment->total_marks }}</p>
        <p class="mb-0"><strong>Deadline:</strong> <span class="{{ $assignment->isExpired() ? 'text-danger' : 'text-success' }}">{{ $assignment->deadline->format('d M Y, h:i A') }}</span></p>
    </div>
</div>
<div class="card">
    <div class="card-body p-0">
        <table class="table table-hover mb-0">
            <thead class="table-light">
                <tr><th>#</th><th>Student</th><th>Answer</th><th>Status</th><th>Grade</th><th>Feedback</th><th>Action</th></tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <a href="{{ route('teacher.assignments.grade.form', [$assignment, $sub]) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil-square"></i> Grade</a>
                        @else

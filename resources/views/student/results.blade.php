...
<div class="row g-3 mb-4">
    <div class="col-md-4">
        <div class="card text-center p-3 shadow-sm border-0">
            <h2 class="text-primary fw-bold">{{ $totalAttemptedCount }}</h2>
            <p class="mb-0 text-muted">Quizzes Attempted</p>
        </div>
    </div>
    <div class="col-md-4">
        <div class="card text-center p-3 shadow-sm border-0">
            <h2 class="text-success fw-bold">{{ $avgQuiz }}%</h2>
            <p class="mb-0 text-muted">Average Quiz Score</p>
        </div>
    </div>
    <div class="col-md-4">
        <div class="card text-center p-3 shadow-sm border-0">
            <h2 class="text-warning fw-bold">{{ $avgAssignment }}</h2>
            <p class="mb-0 text-muted">Average Assignment Grade</p>
        </div>
    </div>
</div>
                        ...
                        <tbody>
                            @forelse($quizAttempts as $a)
                                ...
                                <tr>
                                    ...
                                    @else
                                        {{-- Result NOT Published: Show Placeholder --}}
                        ...
                        <tbody>
                            @forelse($submissions as $sub)
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-bold {{ $sub->grade ? 'text-dark' : 'text-muted italic' }}">
                                            {{ $sub->grade ?? 'Pending' }}
                                        </span>
                                    </td>
                                    <td>

...
<div class="d-flex justify-content-between mb-3">
    <h6 class="text-muted">Total Marks: {{ $quiz->total_marks }} &nbsp;|&nbsp; Attempts: {{ $quiz->attempts->count() }}</h6>
    @if(!$quiz->is_result_published)
    <form action="{{ route('teacher.quizzes.publish', $quiz) }}" method="POST">
        @csrf
        <button class="btn btn-success" onclick="return confirm('Publish results to students?')"><i class="bi bi-send me-1"></i>Publish Results</button>
    </form>
    @else
    ...
    <div class="card-body p-0">
        <table class="table table-hover mb-0">
            <thead class="table-light">
                <tr><th>#</th><th>Student</th><th>Score</th><th>Total</th><th>Percentage</th><th>Grade</th></tr>
            </thead>
            <tbody>

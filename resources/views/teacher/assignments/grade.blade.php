...
<div class="row g-4">
    <div class="col-md-7">
        <div class="card mb-3">
            <div class="card-header bg-white"><strong>Student's Answer</strong></div>
            <div class="card-body">
                <p><strong>Student:</strong> {{ $submission->student->user->name ?? '-' }}</p>
                <hr>
    ...
    <div class="col-md-5">
        <div class="card">
            <div class="card-header bg-white"><strong>Assign Grade</strong></div>
            <div class="card-body">
                <form action="{{ route('teacher.assignments.grade', [$assignment, $submission]) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="grade" class="form-control" min="0" max="{{ $assignment->total_marks }}" value="{{ old('grade', $submission->grade) }}" required>
                    </div>
                    <div class="mb-4">
                        <label class="form-label">Feedback</label>
                        <textarea name="feedback" class="form-control" rows="4" placeholder="Optional feedback for student...">{{ old('feedback', $submission->feedback) }}</textarea>
                    </div>
                    <button type="submit" class="btn btn-success w-100"><i class="bi bi-check-circle me-1"></i>Save Grade</button>
                </form>
            </div>
        </div>
    </div>
